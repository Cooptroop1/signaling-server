const WebSocket = require('ws');
  ...
  // Auto-cleanup for inactive rooms every hour (24h inactivity)
  setInterval(() => {
    const now = Date.now();
    rooms.forEach((room, code) => {
      if (room.clients.size === 0 || (now - room.lastActivity > 86400000)) { // 24 hours in ms
        rooms.delete(code);
        randomCodes.delete(code);
        console.log(`Deleted inactive room ${code}`);
        broadcast(code, {
          type: 'room-deleted',
          message: 'Room deleted due to 24 hours of inactivity.'
        });
      }
    });
    broadcastRandomCodes();
    ...
    ws.on('message', async (message) => {
      ...
      try {
        ...
        if (data.type === 'join') {
          ...
          if (!rooms.has(code)) {
            rooms.set(code, { initiator: clientId, clients: new Map(), maxClients: 2, lastActivity: Date.now() });
          ...
          room.lastActivity = Date.now(); // Update last activity on join
          ws.code = code;
        ...
        if (data.type === 'relay-message' || data.type === 'relay-image' || data.type === 'relay-voice') {
          ...
          room.lastActivity = Date.now(); // Update last activity on message
