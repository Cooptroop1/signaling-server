...
let features = {
  ...
  enableGrokBot: true,
  connectionMode: 'both'  // New: 'both' | 'p2p' | 'relay'
};
...
function validateMessage(data) {
  ...
  switch (data.type) {
    ...
    case 'toggle-feature':
      ...
      break;
    case 'set-connection-mode':  // New validation for mode set
      if (!data.mode || !['both', 'p2p', 'relay'].includes(data.mode)) {
        return { valid: false, error: 'set-connection-mode: invalid mode (both, p2p, relay)' };
      }
      if (!data.secret || typeof data.secret !== 'string') {
        return { valid: false, error: 'set-connection-mode: secret required as string' };
      }
      break;
    case 'ping':
    case 'pong':
...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', async (message) => {
    ...
    try {
      ...
      if (data.type === 'get-stats' || data.type === 'get-features' || data.type === 'toggle-feature' || data.type === 'set-connection-mode') {
      ...
      if (data.type === 'set-connection-mode') {  // New: Handle mode change
        if (data.secret === ADMIN_SECRET) {
          const validModes = ['both', 'p2p', 'relay'];
          if (validModes.includes(data.mode)) {
            features.connectionMode = data.mode;
            saveFeatures();
            const timestamp = new Date().toISOString();
            fs.appendFileSync(LOG_FILE, `${timestamp} - Admin set connectionMode to ${data.mode} by client ${hashIp(clientIp)}\n`);
            ws.send(JSON.stringify({ type: 'feature-toggled', feature: 'connectionMode', mode: data.mode }));
            // Broadcast updated features to all clients
            wss.clients.forEach(client => {
              if (client.readyState === WebSocket.OPEN) {
                client.send(JSON.stringify({ type: 'features-update', ...features }));
              }
            ...
            ws.send(JSON.stringify({ type: 'error', message: 'Invalid connection mode' }));
          }
        } else {
          ws.send(JSON.stringify({ type: 'error', message: 'Invalid admin secret' }));
        }
