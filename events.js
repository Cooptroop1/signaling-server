...
// New: Force relay for 5+ users if not already in relay mode
if (totalClients >= 5 && !useRelay) {
console.log(Group size ${totalClients} reached—switching to relay mode.);
useRelay = true;
// Update UI to reflect relay mode
const privacyStatus = document.getElementById('privacyStatus');
if (privacyStatus) {
privacyStatus.textContent = 'Relay Mode (E2EE)';
privacyStatus.classList.remove('hidden');
}
showStatusMessage('Switched to relay mode for larger group.');
// Optional: Close existing P2P connections to fully switch to relay
