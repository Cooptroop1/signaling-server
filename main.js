// Core logic: peer connections, message sending, handling, etc.
let turnUsername = '';
let turnCredential = '';
...
async function sendMedia(file, type) {
 ...
 const cipher = ciphers.get(targetId);
 if (!cipher) {
 console.error(`No ratchet session for ${targetId}`);
 return;
 }
 const jsonString = JSON.stringify(payload);
 const plaintext = new TextEncoder().encode(jsonString);
 const signed = await cipher.encrypt(plaintext);
 const proto = await signed.exportProto();
 const protoB64 = arrayBufferToBase64(proto);
 dc.send(JSON.stringify({ type: 'ratchet-msg', proto: protoB64 }));
 }
 });
 }
 // Display locally
...
async function startPeerConnection(targetId, isOfferer) {
 ...
 // Send prekey-bundle to targetId
 const bundle = await createPreKeyBundle(identity);
 const bundleB64 = arrayBufferToBase64(bundle);
 sendSignalingMessage('prekey-bundle', { bundle: bundleB64, targetId });
 }
 ...
 const privacyStatus = document.getElementById('privacyStatus');
 if (privacyStatus) {
 privacyStatus.textContent = 'Relay Mode: E2E Encrypted (Limited PFS)';
 privacyStatus.classList.remove('hidden');
 }
 }
...
function setupDataChannel(dataChannel, targetId) {
 ...
 if (data.type === 'ratchet-msg') {
 const protoBuf = base64ToArrayBuffer(data.proto);
 const proto = await DKeyRatchet.SignedMessageProtocol.importProto(protoBuf);
 const cipher = ciphers.get(targetId);
 if (!cipher) {
 console.error(`No ratchet session for ${targetId}`);
 return;
 }
 const decrypted = await cipher.decrypt(proto);
 ...
 console.log(`Invalid message format from ${targetId}:`, payload);
 return;
 }
 if (processedMessageIds.has(payload.messageId)) {
 console.log(`Duplicate message ${payload.messageId} from ${targetId}`);
 return;
 }
 processedMessageIds.add(payload.messageId);
 const senderUsername = usernames.get(targetId) || payload.username;
 const messages = document.getElementById('messages');
 ...
 const payload = data; // For non-ratchet (voice events)
 // Old logic for voice
 const senderUsername = usernames.get(targetId) || payload.username;
 const messages = document.getElementById('messages');
 const isSelf = senderUsername === username;
