...
triggerRatchetPartial(failures, newRoomMaster, newSigningSalt, newMessageSalt, version, 1); // Start retry with count 1
}
} else {
...
userPrivateKey = JSON.stringify(await window.crypto.subtle.exportKey('jwk', keyPair.privateKey));
localStorage.setItem('userPrivateKey', userPrivateKey);
return await exportPublicKey(keyPair.publicKey);
}
async function sendOfflineMessage(toUsername, message) {
const privateKey = await window.crypto.subtle.importKey('jwk', JSON.parse(userPrivateKey), { name: 'ECDH', namedCurve: 'P-384' }, false, ['deriveKey', 'deriveBits']);
...
const importedPublic = await importPublicKey(userPublicKey);
const shared = await deriveSharedKey(ephemeralKeyPair.privateKey, importedPublic);
const { encrypted, iv } = await encryptRaw(shared, message);
socket.send(JSON.stringify({ type: 'offline-message', to: toUsername, encrypted, iv, ephemeral_public: ephemeralPublic, clientId, token }));
}
