  ...
  fs.readFile(filePath, (err, data) => {
    ...
    if (filePath.endsWith('.html')) {
      ...
      let updatedCSP = "default-src 'self'; " +
        `script-src 'self' https://cdnjs.cloudflare.com https://cdn.jsdelivr.net https://unpkg.com 'nonce-${nonce}' 'unsafe-inline'; ` +
...
function validateMessage(data) {
  ...
  switch (data.type) {
    ...
    case 'refresh-token':
      ...
      break;
    case 'prekey-bundle':
      if (!data.bundle || !isValidBase64(data.bundle)) {
        return { valid: false, error: 'prekey-bundle: invalid bundle' };
      }
      if (!data.code) {
        return { valid: false, error: 'prekey-bundle: code required' };
      }
      if (!data.targetId || typeof data.targetId !== 'string') {
        return { valid: false, error: 'prekey-bundle: targetId required as string' };
      }
      break;
    case 'prekey-message':
      if (!data.proto || !isValidBase64(data.proto)) {
        return { valid: false, error: 'prekey-message: invalid proto' };
      }
      if (!data.code) {
        return { valid: false, error: 'prekey-message: code required' };
      }
      if (!data.targetId || typeof data.targetId !== 'string') {
  ...
  ws.on('message', async (message) => {
    ...
    try {
      ...
      Object.keys(data).forEach(key => {
        if (typeof data[key] === 'string' && !(data.type === 'prekey-bundle' && key === 'bundle') && !(data.type === 'prekey-message' && key === 'proto')) {
          data[key] = validator.escape(validator.trim(data[key]));
        }
      });
      if (data.type === 'prekey-bundle' && data.bundle) {
        if (!isValidBase64(data.bundle)) {
          ws.send(JSON.stringify({ type: 'error', message: 'Invalid bundle format' }));
          incrementFailure(clientIp);
          return;
        }
      }
      if (data.type === 'prekey-message' && data.proto) {
        if (!isValidBase64(data.proto)) {
          ws.send(JSON.stringify({ type: 'error', message: 'Invalid proto format' }));
          incrementFailure(clientIp);
          return;
        }
      }
      ...
      if (data.type === 'prekey-bundle' || data.type === 'prekey-message') {
      ...
      if (data.type === 'join') {
        ...
        if (!rooms.has(code)) {
          ...
          ws.send(JSON.stringify({ type: 'init', clientId, maxClients: room.maxClients, isInitiator: false, turnUsername: TURN_USERNAME, turnCredential: TURN_CREDENTIAL, features, existingClients }));
