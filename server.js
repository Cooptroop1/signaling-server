...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', async (message) => {
    ...
    // Re-validate all payloads strictly
    ...
    if (data.type === 'leave') {
      if (rooms.has(data.code)) {
        ...
        if (data.token) {
          ...
          if (data.refreshToken) {
            const decodedRefresh = jwt.verify(data.refreshToken, JWT_SECRET, { ignoreExpiration: true });
            revokedTokens.set(data.refreshToken, decodedRefresh.exp * 1000);
          }
