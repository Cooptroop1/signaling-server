...
let features = {
 ...
 enableGrokBot: true,
 enableTotp: true // New: Toggle for TOTP/2FA rooms
};
...
wss.on('connection', (ws, req) => {
 ...
 if (!features.enableTotp) {
 ws.send(JSON.stringify({ type: 'error', message: 'TOTP rooms are disabled by admin.', code: data.code }));
 return;
 }
 if (rooms.has(data.code) && data.clientId === rooms.get(data.code).initiator) {
