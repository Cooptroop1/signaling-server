    ...
    try {
      ...
      if (data.type === 'refresh-token') {
        ...
        try {
          ...
          await redis.set(`revoked:${data.refreshToken}`, 1, 'PX', oldRefreshExpiry - Date.now());
      ...
      if (data.type === 'public-key') {
        const targetInstance = await redis.get(`client:${data.clientId}`);
        if (targetInstance === instanceId) {
          // Should be initiator, but forward local if needed
        } else {
          pub.publish(`signal:${targetInstance}`, event.data); // Forward raw
        }
        return;
      }
      if (data.type === 'encrypted-room-key') {
        const targetInstance = await redis.get(`client:${data.targetId}`);
        if (targetInstance === instanceId) {
          const targetWs = localClients.get(data.targetId)?.ws;
          if (targetWs && targetWs.readyState === WebSocket.OPEN) {
          ...
          pub.publish(`signal:${targetInstance}`, event.data);
        }
        return;
      }
      ...
      if (data.type === 'join') {
        ...
        let roomExists = await redis.exists(roomKey);
        let room;
        if (!roomExists) {
          ...
          room = JSON.parse(await redis.get(roomKey));
          ...
          if (members.includes(clientId)) {
            ...
            if (existingUsername === username) {
              // Reconnect, close old
              // But since ws new, old ws may be closed by ping
              // Update instance
            } else {
...
// IP rate limiting function: max 5 actions (join/submit) per minute per IP (shared via Redis)
...
// Daily IP limit for joins (100/day) (shared)
...
// Increment failure count and ban IP with exponential duration if threshold reached (shared)
