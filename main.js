...
async function prepareAndSendMessage({ content, type = 'message', file = null, base64 = null }) {
  ...
  const { encrypted, iv, nonce: returnedNonce } = await encryptRaw(messageKey, rawData, nonce); // Updated to use nonce
  const toSign = rawData + returnedNonce; // Updated: Sign rawData + nonce (timestamp is inside rawData)
  const signature = await signMessage(signingKey, toSign);
  let payload = { messageId, nonce: returnedNonce, iv, signature, encryptedBlob: encrypted }; // New: Use encryptedBlob instead
  ...
  processedNonces.set(returnedNonce, Date.now()); // Changed to Map: nonce -> timestamp
  messageCount++;
...
async function processReceivedMessage(data, targetId) {
  ...
  try {
    const messageKey = await deriveMessageKey();
    const rawData = await decryptRaw(messageKey, data.encryptedBlob, data.iv, data.nonce); // Updated to pass nonce
...
async function sendOfflineMessage(toUsername, messageText) {
  ...
  const nonce = crypto.randomUUID(); // Generate nonce for offline message
  const { encrypted, iv, nonce: returnedNonce } = await encryptRaw(shared, messageText, nonce); // Updated to use nonce
  ...
  socket.send(JSON.stringify({
    ...
    nonce: returnedNonce, // Include nonce in payload
    clientId,
    token
  }));
