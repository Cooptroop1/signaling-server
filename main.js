// main.js // Core logic: peer connections, message sending, handling offers, etc. 
let turnUsername = ''; 
let turnCredential = ''; 
...
let messageCount = 0; 
let ratchets = new Map(); // targetId => DoubleRatchet 
...
async function startPeerConnection(targetId, isOfferer) { 
  ...
  peerConnection.ontrack = (event) => { 
    ...
    if (!remoteAudios.has(targetId)) { 
      ...
      audio.autoplay = true; // Default to lower volume for earpiece simulation 
...
function setupDataChannel(dataChannel, targetId) { 
  ...
  dataChannel.onmessage = async (event) => { 
    ...
    if (data.type === 'encrypted') { 
      await handleEncryptedMessage(targetId, data.header, data.ciphertext, data.iv); 
    } else if (data.type === 'new-room-key') { 
      // Handle PFS key update in P2P mode if needed 
      try { 
        ...
        const newRoomMasterBuffer = await decryptBytes(shared, data.encrypted, data.iv); 
...
async function handleEncryptedMessage(senderId, header, ciphertext, iv) { 
  const ratchet = ratchets.get(senderId); 
  if (!ratchet) { 
    console.warn(`No ratchet for sender ${senderId}`); 
    return; 
  } 
  const plaintext = await ratchet.ratchetDecrypt(header, ciphertext, iv); 
  const payload = JSON.parse(plaintext); 
  if (!payload.messageId || !payload.username || (!payload.content && !payload.data)) { 
    console.log(`Invalid message format from ${senderId}:`, payload); 
    return; 
  } 
  if (processedMessageIds.has(payload.messageId)) { 
    console.log(`Duplicate message ${payload.messageId} from ${senderId}`); 
    return; 
  } 
  processedMessageIds.add(payload.messageId); 
  const senderUsername = usernames.get(senderId) || payload.username; 
  const messages = document.getElementById('messages'); 
  ...
  if (isInitiator) { 
    dataChannels.forEach((dc, id) => { 
      if (id !== senderId && dc.readyState === 'open') { 
...
function toggleGrokBot() { 
  ...
  if (grokBotActive) { 
    if (!grokApiKey) { 
      ...
      showStatusMessage('Grok bot enabled. Use /grok to ask questions.'); 
    } 
  } else { 
...
function saveGrokKey() { 
  ...
  if (grokApiKey) { 
    ...
    showStatusMessage('API key saved. Use /grok to ask Grok.'); 
    keyInput.value = ''; 
  } else { 
