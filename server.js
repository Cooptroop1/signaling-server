...
const clientIp = req.headers['x-forwarded-for'] || ws.socket.remoteAddress; // Handle proxies
...
room.clients.forEach((, existingClientId) => {
...
ws.send(JSON.stringify({ type: 'error', message: 'Code already submitted' }));
incrementFailure(clientIp);
return;
}
...
if (payload && payload.length > 13653) { // ~10KB base64 = 13653 chars (1010244/3)
