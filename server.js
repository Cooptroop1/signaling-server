...
setInterval(() => wsHandlers.pingClients(wss), 50000);
setInterval(async () => await redisUtils.cleanupRevokedTokens(redis), 3600000);

// Pub/sub handler
sub.on('message', (channel, event) => wsHandlers.handlePubSub(channel, event, localRooms, localClients, wss, instanceId, features, pub));

// WebSocket connection handler
wss.on('connection', (ws, req) => wsHandlers.handleConnection(ws, req, features, localClients, localRooms, redis, pub, instanceId, validation, statsLogger, config, otplib));
