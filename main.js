...
function startPeerConnection(targetId, isOfferer) {
  ...
  const timeout = setTimeout(() => {
    if (!dataChannels.get(targetId) || dataChannels.get(targetId).readyState !== 'open') {
      console.log(`P2P failed with ${targetId}`);
      if (features.connectionMode === 'both') {
        useRelay = true;
        ...
        showStatusMessage('P2P connection failed. No relay available in P2P-only mode.');
        cleanupPeerConnection(targetId);
      }
      // For 'relay' mode, already skipped
    }
  }, 10000); // 10s timeout for fallback (only if 'both')
...
function updateFeaturesUI() {
  ...
  if (voiceCallButton) {
    const hideVoiceCall = !features.enableVoiceCalls || features.connectionMode === 'relay';
    voiceCallButton.classList.toggle('hidden', hideVoiceCall);
    voiceCallButton.title = hideVoiceCall ? 'Voice calls disabled' : 'Start Voice Call';
    if (hideVoiceCall && voiceCallActive) {
      stopVoiceCall(); // Force stop call if disabled
    }
  }
  if (audioOutputButton) {
    const shouldHide = !features.enableAudioToggle || !voiceCallActive || features.connectionMode === 'relay' || !features.enableVoiceCalls;
